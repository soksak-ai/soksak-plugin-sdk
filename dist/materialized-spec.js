...
function treeSha256(root, at = root, prefix = "", hash = createHash("sha256")) {
    for (const entry of readdirSync(at, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
        if (prefix === "" && (entry.name === ".soksak-dependency.json" || entry.name === ".soksak-release.json"))
            continue;
        const path = join(at, entry.name);
        const relative = prefix ? `${prefix}/${entry.name}` : entry.name;
        const stat = lstatSync(path);
        if (stat.isSymbolicLink())
            throw new Error(`materialized Spec contains a symbolic link: ${relative}`);
        if (stat.isDirectory())
            treeSha256(root, path, relative, hash);
        else if (stat.isFile() && realpathSync(path) === path)
            hash.update(relative).update("\0").update(readFileSync(path)).update("\0");
        else
            throw new Error(`materialized Spec contains a non-regular entry: ${relative}`);
    }
    return prefix === "" ? hash.digest("hex") : "";
}
function metadata(specRoot) {
    if (!isAbsolute(specRoot) || !existsSync(specRoot))
        throw new Error("Spec root must be an absolute directory");
    const stat = lstatSync(specRoot);
    if (!stat.isDirectory() || stat.isSymbolicLink() || realpathSync(specRoot) !== specRoot)
        throw new Error("Spec root must be a regular directory");
    ...
    if (pkg.name !== "@soksak/soksak-spec" || typeof pkg.version !== "string" ||
...
export function materializedSpecRelease(specRoot) {
...
export function materializedSpecScript(specRoot, relative) {
    metadata(specRoot);
