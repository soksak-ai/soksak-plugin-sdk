...
function componentManifest(kind, id, version) {
    if (kind === "plugin") {
        return {
...
export function scaffoldComponent(input) {
    if (!COMPONENT_KINDS.includes(input.kind) || !ID.test(input.id) || !VERSION.test(input.version))
        throw new Error("scaffold identity is invalid");
    if (!isAbsolute(input.out) || existsSync(input.out))
        throw new Error("scaffold output must be a new absolute directory");
    const parent = dirname(input.out);
    const stat = lstatSync(parent);
    if (!stat.isDirectory() || stat.isSymbolicLink() || realpathSync(parent) !== parent)
        throw new Error("scaffold parent must be a regular directory");
    const stage = join(parent, `.${basename(input.out)}.next-${process.pid}`);
    if (existsSync(stage))
        throw new Error("scaffold staging directory already exists");
    mkdirSync(stage);
    try {
        ...
        if (input.kind === "plugin") {
            ...
            write(join(stage, "package.json"), {
                ...
                peerDependencies: { "@soksak/soksak-sdk": "0.0.6", "@soksak/soksak-spec": "0.0.37" },
            });
