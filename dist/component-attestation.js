...
import { createComponentBuildReceipt } from "./component-tools.js";
...
function toolingRelease(path, sdkVersion) {
...
export function attestComponentRelease(input) {
    ...
    const previous = Array.isArray(release.evidence) ? release.evidence.find((item) => item?.file === receiptName) : undefined;
    ...
    const evidence = [...(release.evidence ?? []).filter((item) => item?.file !== receiptName), reference]
    ...
    try {
        ...
        if (!existsSync(receiptPath))
            renameSync(receiptNext, receiptPath);
        else
            rmSync(receiptNext);
        ...
        rmSync(receiptNext, { force: true });
        rmSync(releaseNext, { force: true });
    }
}
