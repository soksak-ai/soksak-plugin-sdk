...
test("GitHub adapter peels tags and audits every asset page", async () => {
  ...
  const api = new GitHubApi({
    ...
    token: "installation-token",
    fetchImpl: async (url) => {
      if (url.endsWith("/git/ref/tags/soksak-plugin-sdk-v0.9.7")) return reply(200, { object: { type: "tag", sha: annotated } });
  ...
  assert.equal(await api.getTagCommit("soksak-plugin-sdk-v0.9.7"), commit);
...
test("GitHub adapter creates a draft, uploads bytes, and publishes without replacing assets", async () => {
  ...
  await api.createTag("soksak-plugin-sdk-v0.9.7", "a".repeat(40));
  await api.createDraft("soksak-plugin-sdk-v0.9.7", "a".repeat(40));
  await api.uploadAsset({ id: 7 }, { name: "asset.tgz", bytes, contentType: "application/gzip" });
  await api.publishDraft({ id: 7 }, false);
  assert.deepEqual(JSON.parse(calls[0].options.body), { ref: "refs/tags/soksak-plugin-sdk-v0.9.7", sha: "a".repeat(40) });
