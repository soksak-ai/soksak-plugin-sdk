...
function sameIdentity(left: SidecarManifest, right: SidecarManifest, target: string): boolean {
...
export function packSidecarTarget(input: { root: string; specRoot: string; target: string; source: string; out: string }): Record<string, unknown> {
  ...
  if (!isAbsolute(input.out)) throw new Error("target archive output must be absolute");
...
export function packageSidecarRelease(input: {
  root: string; specRoot: string; commit: string; artifacts: string; out: string;
  ...
  try {
    ...
    for (const item of release.artifacts) {
      if (!item || typeof item.file !== "string") throw new Error("generated Sidecar artifact reference is invalid");
      const source = assertRegularFile(join(artifacts, item.file), `Sidecar artifact ${item.file}`);
      const destination = join(stage, item.file);
      if (existsSync(destination)) throw new Error(`duplicate Sidecar release file: ${item.file}`);
